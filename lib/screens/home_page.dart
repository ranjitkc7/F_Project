...
class _HomePageState extends State<HomePage> {
  ...
  List<StudentDetails> get studentsForSelectedClass {
    return students.where((s) {
      final matchesClass =
          selectedClass == null || s.studentClass == selectedClass;

      final matchesSearchClass =
          searchClass == null || s.studentClass == searchClass;

      final matchesName = s.name.toLowerCase().contains(
        searchQuery.toLowerCase(),
      );

      return matchesClass && matchesSearchClass && matchesName;
    }).toList();
  }
              ...
              child: Column(
                ...
                children: [
                  ...
                  CustomButton(
                    ...
                    onPressed: () async {
                      Navigator.pop(context);
                      final updateStudent = await Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => UpdateStudentPage(student: s),
                        ),
                      );
                      if (updateStudent != null) {
                        setState(() {
                          final index = students.indexWhere(
                            (e) => e.id == updateStudent.id,
                          );
                          ...
                          child: ListTile(
                            ...
                            onTap: () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (_) => StudentQrPage(student: s),
                                ),
                              );
                            },
                          ),
